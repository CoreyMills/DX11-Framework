...
{
	if (!_gameObjectArray.empty()) _gameObjectArray.erase(_gameObjectArray.begin(), _gameObjectArray.end());
	if (!_textureArray.empty()) _textureArray.erase(_textureArray.begin(), _textureArray.end());
	if (!_meshArray.empty()) _meshArray.erase(_meshArray.begin(), _meshArray.end());
}

void ResourceManager::Add3DObject(GameObject * object)
{
	_gameObjectArray.push_back(object);
}

void ResourceManager::RemoveObjectAt(int index)
{
	//Deletes element at index
	_gameObjectArray.erase(_gameObjectArray.begin() + index);
	_gameObjectArray.shrink_to_fit();
}

void ResourceManager::RemoveAllObjects()
{
	//Deletes elements between first and second parameter
	_gameObjectArray.erase(_gameObjectArray.begin(), _gameObjectArray.end());
	_gameObjectArray.shrink_to_fit();
}

GameObject* ResourceManager::GetObjectByIndex(int index)
{
	return _gameObjectArray.at(index);
}

GameObject* ResourceManager::GetObjectByName(std::string name)
{
	for (unsigned int i = 0; i < _gameObjectArray.size(); i++)
	{
		if (_gameObjectArray.at(i)->GetName() == name)
		{
			return _gameObjectArray.at(i);
		}
	}
	return nullptr;
}
...
////Texture Methods////////////////////////////////
HRESULT ResourceManager::LoadTexture(ID3D11Device& _pd3dDevice, const wchar_t* fileName)
{
	ID3D11ShaderResourceView* textureRV;

	HRESULT hr;
	hr = CreateDDSTextureFromFile(&_pd3dDevice, fileName, nullptr, &textureRV);
	if (FAILED(hr))
	{
		return hr;
	}

	_textureArray.push_back(textureRV);
	return S_OK;
}

void ResourceManager::RemoveTextureAt(int index)
{
	//Deletes the element at index
	_textureArray.erase(_textureArray.begin() + index);
	_textureArray.shrink_to_fit();
}

void ResourceManager::RemoveAllTextures()
{
	//Deletes elements between first and second parameter
	_textureArray.erase(_textureArray.begin(), _textureArray.end());
	_textureArray.shrink_to_fit();
}

ID3D11ShaderResourceView* ResourceManager::GetTextureAt(int index)
{
	return _textureArray.at(index);
}
...
void ResourceManager::LoadOBJ(std::string meshName, char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if (!binaryInFile.good())
	{
		...
		{
			Mesh* temp = new Mesh();
			temp->meshName = meshName;
			_meshArray.push_back(temp);
		}
		else
		{
			...
			meshData->offsetVB = 0;
			meshData->strideVB = sizeof(SimpleVertex);
...
void ResourceManager::AddOBJ(Mesh * newMesh)
{
	_meshArray.push_back(newMesh);
}

bool ResourceManager::FindSimilarVertex(const SimpleVertex& vertex, std::map<SimpleVertex, unsigned short>& vertToIndexMap, unsigned short& index)
{
...
void ResourceManager::CreateIndices(const std::vector<XMFLOAT3>& inVertices, const std::vector<XMFLOAT2>& inTexCoords, const std::vector<XMFLOAT3>& inNormals,
	std::vector<unsigned short>& outIndices, std::vector<XMFLOAT3>& outVertices, std::vector<XMFLOAT2>& outTexCoords, std::vector<XMFLOAT3>& outNormals)
{
...
void ResourceManager::RemoveMeshAt(int index)
{
	//Deletes the element at index
	_meshArray.erase(_meshArray.begin() + index);
	_meshArray.shrink_to_fit();
}

void ResourceManager::RemoveAllMeshs()
{
	//Deletes elemnts between first and second parameter
	_meshArray.erase(_meshArray.begin(), _meshArray.end());
	_meshArray.shrink_to_fit();
}

Mesh* ResourceManager::GetMeshByIndex(int index)
{
	return _meshArray.at(index);
}

Mesh* ResourceManager::GetMeshByName(std::string meshName)
{
	for (unsigned int i = 0; i < _meshArray.size(); i++)
	{
		if (_meshArray.at(i)->meshName == meshName)
		{
			return _meshArray.at(i);
		}
	}
	return nullptr;
}